import { readFile } from "node:fs/promises";const API_URL = "https://api.buffer.com";const apiKey = process.env.BUFFER_API_KEY?.trim().replace(/^[\u201C\u201D\"'\x60]+|[\u201C\u201D\"'\x60]+$/g, "");if (!apiKey) throw new Error("Missing required environment variable: BUFFER_API_KEY");async function buffer(query, variables = {}) {  const response = await fetch(API_URL, {    method: "POST",    headers: {      "Content-Type": "application/json",      Authorization: `Bearer ${apiKey}`,    },    body: JSON.stringify({ query, variables }),  });  const result = await response.json();  if (!response.ok || result.errors) {    throw new Error(`Buffer API request failed: ${JSON.stringify(result)}`);  }  return result.data;}async function findInstagramChannel() {  if (process.env.BUFFER_CHANNEL_ID) return process.env.BUFFER_CHANNEL_ID;  const account = await buffer(`query GetOrganizations {    account { organizations { id name } }  }`);  const organizations = account.account?.organizations ?? [];  const channels = [];  for (const organization of organizations) {    const data = await buffer(      `query GetChannels($organizationId: OrganizationId!) {        channels(input: { organizationId: $organizationId }) {          id name displayName service        }      }`,      { organizationId: organization.id },    );    channels.push(...data.channels);  }  const wanted = (process.env.BUFFER_CHANNEL_NAME || "theballdept")    .toLowerCase()    .replace(/^@/, "");  const matches = channels.filter((channel) => {    const names = [channel.name, channel.displayName]      .filter(Boolean)      .map((name) => name.toLowerCase().replace(/^@/, ""));    return channel.service === "instagram" && names.includes(wanted);  });  if (matches.length !== 1) {    const instagram = channels      .filter((channel) => channel.service === "instagram")      .map((channel) => `@${channel.name || channel.displayName} (${channel.id})`)      .join(", ");    throw new Error(      `Could not uniquely find Instagram channel @${wanted}. Connected Instagram channels: ${instagram || "none"}`,    );import { readFile } from "node:fs/promises";
